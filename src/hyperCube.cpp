...
HyperCube::HyperCube(int k, int M, int probes, Data &data, float w)
...
int HyperCube::Run(vector<pair<string,vector<float>>> &queries, ofstream &outputFileint, int &N, int &R)
{
    for (int i = 0; i < int(queries.size()); i++)
    {
        ...
        vector<pair<int, int>> cubeResult = this->exec_query(queries[i].second, N);
...
void HyperCube::hashData()
{
    for (int i = 0; i < this->data.n; i++)
    {
        ...
        {
            s +=to_string(this->fTable[j]->calculate_f(to_string(this->ht->calculate_h(this->data.data[i].second, this->ht->v[j],this->k))));
        }

        this->HyperCube_Insert(s,i, this->data.data[i].second);
        
    }
    
}

void HyperCube::HyperCube_Insert(string &s, int index, vector<float> &point)
{
...
list<string> HyperCube::Hamming_Distance(string s, int probes)
{
    list<string> l;
    string curr;
    int level =1;
